...
export const mapProject = (project) => ({
  ...
  initial_status: project.initial_status || "draft",

  assigned_professional: project.assignedProfessional
    ? {
        id: project.assignedProfessional.id,
        name: project.assignedProfessional.name,
        role: project.assignedProfessional.role,
        trade: project.assignedProfessional.trade || null,
        avatar: project.assignedProfessional.avatar || null,
      }
    : null,

  project_team: project.projectTeam?.map((member) => ({
    id: member.user.id,
    name: member.user.name,
    role: member.user.role,
    trade: member.user.trade || null,
...
//   project_team: project.projectTeam.map(member => member.user).map((member) => ({
//     id: member.id,
//     name: member.name,
//     role: member.role,
